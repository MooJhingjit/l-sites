...
import { Columns3Icon, LayoutListIcon } from "lucide-react";
...
export default async function LeadPage({ params, searchParams }: Props) {
  ...
  if (
    ...
    ["board", "table"].includes(searchParams.display)
  ) {
      ...
      <Tabs defaultValue={defaultDisplay} className="">
        <div className="flex items-center ">
          <TabsList className="rounded-sm">
            <TabsTrigger
              value="board"
              className="text-xs space-x-1.5 rounded-sm px-1.5 mx-1"
            >
        ...
        <TabsContent
          value="board"
          className="relative overflow-x-scroll w-full"
        >
          <LeadBoardView />
        </TabsContent>
        <TabsContent value="table" className="relative">
          <LeadTableView columns={columns} data={defaultLeads} />
        </TabsContent>
      </Tabs>
    </PageLayout>
  );
